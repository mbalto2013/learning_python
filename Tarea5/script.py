...
def realizar_encuentros(nombre,cartas_jugador,cartas_ocultas,palos,juegos_por_ganar):
    ...
    for indice in range(0,10):
        ...
        print(f'  {carta_jugador.informacion()} vs {carta_oculta.informacion()} = {resultado.get("msj")}')

    victorias = len([ resultado for resultado in resulado_de_encuetros if resultado.get('puntaje') == 1 ])
...
def iniciar_juego(nombre):
    ...
    encuentros_estimados = int(input('De diez encuetros, cuantos cree ganar? (1-10): '))
    
    if encuentros_estimados in range(1,11):
        realizar_encuentros(nombre,cartas_del_jugador, cartas_ocultas,palos,encuentros_estimados)
    else:
        print('Error')
